...
void pa_volume_memchunk( pa_memchunk*c, const pa_sample_spec *spec, const pa_cvolume *volume) {
    ...
    for (channel = 0; channel < nchannels; channel++) {
        double v = pa_sw_volume_to_linear(volume->values[channel]);
        if (spec->format == PA_SAMPLE_FLOAT32LE || spec->format == PA_SAMPLE_FLOAT32BE)
            linear[channel].f = (float) v;
        else
            linear[channel].i = (int32_t) lrint(v * 0x10000);
    }
    for (padding = 0; padding < VOLUME_PADDING; padding++, channel++)
        linear[channel].i = linear[padding].i;
